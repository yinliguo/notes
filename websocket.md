# WebSocket
> 翻译自[RFC6455](https://tools.ietf.org/html/rfc6455)、[RFC7692](https://tools.ietf.org/html/rfc7692)
...
### 压缩数据帧
* 当启用压缩扩展时，数据帧的第一帧的RSV1标志消息是否被压缩。
* 发送端把消息进行压缩，然后（分帧）发送，接收端把所有帧的payload取出来串起来进行解压

#### 1、压缩
* 压缩的消息可能来自应用程序，也可能来自另外一个扩展的输出
* 最后一个压缩扩展把数据压缩后，设置第一个帧的RSV1为1；如果发送不压缩的消息，就把RSV1置为0

#### 2、解压
* 把收到的分帧中的payload组合起来进行解压

#### 3、permessage-deflate扩展
##### 3.1、参数
* `server_no_context_takeover`客户端在扩展协商时可能提供这个参数（该参数无值），表示阻止服务器使用context takeover，如果服务器不使用context takeover，客户端就没必要保留内存维持消息间的LZ77滑动窗口。如果没有这个参数，表示客户端能够解压服务器使用context takeover构建的消息。如果提供了这个参数，服务器返回这个参数表示接受了协商条件。推荐服务器支持这个参数。即使没有提供这个参数，服务器也可能返回这个参数
* `client_no_context_takeover`这个参数没有值。代表客户端通知服务器：如果你不返回这个参数，我就不用context takeover。如果客户端提供了这个参数，服务器可能忽略它或使用它来避免取代LZ77滑动窗口，阻止客户端使用context takeover。这样会减少服务器为连接保留的内存。如果没有这个参数，表示服务器能解压客户端使用context takeover构建的消息。客户端*必须*支持响应中的这个参数。
* `server_max_window_bits`在协商过程中，客户端可能提供这个参数。这个参数有一个8-15的十进制数（没有前导零）。如果提供了这个参数，表示客户端限制服务器压缩消息时使用的LZ77滑动窗口的大小。如果服务器使用小的滑动窗口压缩消息，客户端就减少为LZ77滑动窗口保留的内存。如果服务器不支持这个参数，就拒绝。如果客户端不提供这个参数，表示客户端能接收最大使用32768字节的LZ77滑动窗口压缩的消息。服务器如果想接受这个参数，就返回参数值小于等于原值的相同参数。即使没有提供这个参数，服务器也可以返回这个参数。
* `client_max_window_bits`这个参数可能没有值或取值与上面相同。如果提供了这个参数，表示客户端通知服务器：我支持这个参数。如果客户端给这个参数设置了值，表示通知服务器：如果你不返回这个参数，或者返回的参数的值不大于原值，我就不会使用比原值大的LZ77滑动窗口来压缩消息。如果服务器接收了这个参数，可能会忽略它，或者使用一个小于等于原值的值来避免不必要的太大的LZ77滑动窗口。如果客户端没有发送这个参数，服务器绝不能响应这个参数。如果服务器没有响应这个参数，表示服务器能接受最大为32768字节的LZ77滑动窗口压缩的消息。
